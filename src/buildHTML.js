...
var isBinLeftCanceller = function(node, isComplete) {
    if (node) {
...
var isBinRightCanceller = function(node, isComplete) {
    if (node) {
 ...
 * returned list contains no fragments. `isComplete` is true if `expression`
 * is complete (no atoms will be added on either side).
 */
var buildExpression = function(expression, options, isComplete) {
    ...
    for (i = 0; i < groups.length; i++) {
        if (isBin(groups[i])
            && (isBinLeftCanceller(groups[i - 1], isComplete)
                || isBinRightCanceller(groups[i + 1], isComplete))) {
